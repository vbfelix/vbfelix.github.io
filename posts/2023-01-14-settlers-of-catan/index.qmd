...
A disclaimer: this dataset has only 50 observations, with one of the players always being the same, and with a winrate of 50%, so the goal here is simply to look at the data and test some hypotheses, rather than to do an inference or study about the game.

## Is the dice fair?

The first analysis will about the dice rolls, every turn each player roll 2 dices and use the sum of them so the respective location will give resources.
...
1.  The loss and gain are positive correlated;

2.  There is no player that lost more than gained, as no point is below the identity line;

3.  The winner gained a lot more than player that lost.
...
-   The losers have a more concentrated density, using the peak of the density as a metric, winners gain 2.31 cards to every card lost, whereas losers gain 2.15 cards.

## More Cards = More Points?

Even though we have a lot of metric about cards gain and loss, what matters to win are the points, so let's take a look how correlated are they.

```{r}
catan_df %>% 
  select(points,production:totalLoss) %>% 
  pivot_longer(cols = -points) %>% 
  group_by(name) %>% 
  summarise(corr = cor(points,value)) %>% 
  arrange(-corr)
