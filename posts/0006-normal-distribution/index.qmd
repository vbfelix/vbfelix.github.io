...
ggplot(data = data.frame(x = c(-3, 3)), aes(x, col = "1.00")) +
  ...
  geom_vline(xintercept = 0,col = "red",linetype = "dashed")+
  plt_theme_x()+
  scale_color_manual(values = palette_seq(name = "cyberpunk"))+
  plt_water_mark(vfx_watermark)
```
...
A simple application is used in my previous post, where we see that the [sum of two dices is approximately a normal distribution](https://vbfelix.github.io/posts/2023-01-14-settlers-of-catan/#is-the-dice-fair).
