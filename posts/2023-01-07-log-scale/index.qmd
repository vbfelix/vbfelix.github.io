...
date: "2023-01-07"
categories: [Intro to, Theory, DataViz]
...
To see that, let's use the @eq-log-formula and define a second equation as:
...
Now, if we use the @eq-exp-product to @eq-log-product, we result in:
...
if we use the @eq-exp-quotient to @eq-log-quotient, we result in:
